...
public class ProdutoService {
    ...
    public List<Produto> listar() { return repository.findAll(Sort.by("nome").ascending()); }
    ...
    public Produto salvar(Long id, ProdutoForm form) {
        ...
        produto.setDescricao(form.getDescricao() == null ? "" : form.getDescricao().strip());
    ...
    private void validar(ProdutoForm form) {
        if (form.getNome() == null || form.getNome().isBlank() || form.getNome().strip().length() > 100)
            throw new IllegalArgumentException("Informe um nome com 1 a 100 caracteres.");
        ...
        if (form.getPreco() == null || form.getPreco().signum() <= 0
                || form.getPreco().compareTo(new BigDecimal("9999999999.99")) > 0
                || form.getPreco().stripTrailingZeros().scale() > 2)
            throw new IllegalArgumentException("Informe um preço positivo, de até 9.999.999.999,99, com no máximo duas casas decimais.");
        if (form.getQuantidade() == null || form.getQuantidade() < 0)
            throw new IllegalArgumentException("Informe uma quantidade inteira maior ou igual a zero.");
    }
}
